...
public class App {
	...
	private void handleBuild(String[] args) throws Exception {
		...
		/**
		 * To store the Index directory
		 */
		Time.init(args[0], true, System.out);
	...
	private void handleSearch(String[] args) throws Exception {
		...
		double[] arr = new double[30000000];
		...
		File file = new File(Time.path + Time.TTIndex);
		if(file.exists()){
			...
			//System.out.println(Time._Map.size() + " " + Time._keys.size());

			ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(new File(Time.path + "/TimeTupleIndex")));
        	...
        	PrintStream writer = new PrintStream(new File(Time.path + "/Dump2"));
        	for(Map.Entry<String, String> iter : Time._perfectMap.entrySet()){
